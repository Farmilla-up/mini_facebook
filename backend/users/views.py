...
from .serializer import UserSerializer, AddUserSerializer, ChangeUserSerializer, EmptySerializer
    ...
    def post(self,request, *args, **kwargs):
        try :
            if kwargs['action'] == 'accept':
                ...
                friend_request =  Friendship.objects.filter(to_user = user, from_user = from_who).first()
                ...
                return Response({"status": "Friend request denied. Subscription remains."})

            else :
                return Response({"error": "Invalid action. Use 'accept' or 'deny'."}, status=400)
        except Exception as e :
            return Response({"error" : str(e)})
...
class SubscribeView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if existing:
                return Response({"message": "Already sent request or already subscribed."}, status = status.HTTP_400_BAD_REQUEST)
...
class SubscribesListView(RetrieveAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        user = self.get_object()
        sent = Friendship.objects.filter(from_user=user, accepted=False).values_list('to_user', flat=True)
...
class SubscribersListView(RetrieveAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        try :
            user = self.get_object()
            sent = Friendship.objects.filter(to_user=user, accepted=False).values_list('from_user', flat=True)
            users = User.objects.filter(id__in=sent)
...
class DeleteFriendView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            other = friendship.from_user if friendship.from_user != user else friendship.to_user
            ...
            return Response({"status": "Unfriended. Now one-way subscription."}, status=status.HTTP_200_OK)
